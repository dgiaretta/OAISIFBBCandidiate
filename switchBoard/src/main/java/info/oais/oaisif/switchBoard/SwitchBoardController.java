...
@RestController
@RequestMapping(value="/oaisif/v1/switchboard", produces = "application/json; charset=UTF-8")
public class SwitchBoardController {
    ...
    public ResponseEntity<String> getByArchiveNameLikeByRequestParam(@PathVariable(value = "name") String name) {
    	...
    	if (ar == null || ar.isEmpty() || ar.size() < 1 ) { 
    		return ResponseEntity.status(HttpStatus.NOT_FOUND) .body("{\"error\":\"Entry not found for name: " + name + "\"}"); 
    	} 
    ...
    @ApiResponses(value = { 
            ...
            @ApiResponse(responseCode = "401", description = "Unauthorized access", content = @Content), 
            @ApiResponse(responseCode = "403", description = "Forbidden request", content = @Content), 
            @ApiResponse(responseCode = "404", description = "Switchboard list not found", content = @Content),
            @ApiResponse(responseCode = "405", description = "Method Not Allowed", content = @Content)
            })
    @GetMapping(value="/sources", produces = "application/json")
    public ResponseEntity<String> getBySBAll(
    	...
    	Page<SwitchBoardEntry> ar = switchBoardRepository.findByArchiveDescriptionContains(query, pageable);
